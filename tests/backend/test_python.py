import ast

import astunparse
import pytest

from moa.ast import MOANodeTypes, Node, SymbolNode
from moa.backend import generate_python_source


@pytest.mark.parametrize('symbol_table, tree, expected_source', [
    # (+-/*)
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (3, 4), None),
      'B': SymbolNode(MOANodeTypes.ARRAY, (3, 4), None)},
     Node(MOANodeTypes.PLUS, (3, 4),
          Node(MOANodeTypes.ARRAY, (3, 4), 'A'),
          Node(MOANodeTypes.ARRAY, (3, 4), 'B')),
     '(A + B)'),
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (3, 4), None),
      'B': SymbolNode(MOANodeTypes.ARRAY, (3, 4), None)},
     Node(MOANodeTypes.MINUS, (3, 4),
          Node(MOANodeTypes.ARRAY, (3, 4), 'A'),
          Node(MOANodeTypes.ARRAY, (3, 4), 'B')),
     '(A - B)'),
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (3, 4), None),
      'B': SymbolNode(MOANodeTypes.ARRAY, (3, 4), None)},
     Node(MOANodeTypes.TIMES, (3, 4),
          Node(MOANodeTypes.ARRAY, (3, 4), 'A'),
          Node(MOANodeTypes.ARRAY, (3, 4), 'B')),
     '(A * B)'),
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (3, 4), None),
      'B': SymbolNode(MOANodeTypes.ARRAY, (3, 4), None)},
     Node(MOANodeTypes.DIVIDE, (3, 4),
          Node(MOANodeTypes.ARRAY, (3, 4), 'A'),
          Node(MOANodeTypes.ARRAY, (3, 4), 'B')),
     '(A / B)'),
    # ==, !=, <, >, <=, >=
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (), (4,)),
      'B': SymbolNode(MOANodeTypes.ARRAY, (), (3,))},
     Node(MOANodeTypes.EQUAL, (),
          Node(MOANodeTypes.ARRAY, (), 'A'),
          Node(MOANodeTypes.ARRAY, (), 'B')),
      '(A == B)'),
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (), (4,)),
      'B': SymbolNode(MOANodeTypes.ARRAY, (), (3,))},
     Node(MOANodeTypes.NOTEQUAL, (),
          Node(MOANodeTypes.ARRAY, (), 'A'),
          Node(MOANodeTypes.ARRAY, (), 'B')),
      '(A != B)'),
     ({'A': SymbolNode(MOANodeTypes.ARRAY, (), (4,)),
       'B': SymbolNode(MOANodeTypes.ARRAY, (), (3,))},
      Node(MOANodeTypes.LESSTHAN, (),
           Node(MOANodeTypes.ARRAY, (), 'A'),
                Node(MOANodeTypes.ARRAY, (), 'B')),
      '(A < B)'),
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (), (4,)),
       'B': SymbolNode(MOANodeTypes.ARRAY, (), (3,))},
     Node(MOANodeTypes.LESSTHANEQUAL, (),
                Node(MOANodeTypes.ARRAY, (), 'A'),
                Node(MOANodeTypes.ARRAY, (), 'B')),
      '(A <= B)'),
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (), (4,)),
       'B': SymbolNode(MOANodeTypes.ARRAY, (), (3,))},
     Node(MOANodeTypes.GREATERTHAN, (),
          Node(MOANodeTypes.ARRAY, (), 'A'),
          Node(MOANodeTypes.ARRAY, (), 'B')),
     '(A > B)'),
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (), (4,)),
      'B': SymbolNode(MOANodeTypes.ARRAY, (), (3,))},
     Node(MOANodeTypes.GREATERTHANEQUAL, (),
          Node(MOANodeTypes.ARRAY, (), 'A'),
          Node(MOANodeTypes.ARRAY, (), 'B')),
     '(A >= B)'),
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (), (4,)),
      'B': SymbolNode(MOANodeTypes.ARRAY, (), (3,))},
     Node(MOANodeTypes.GREATERTHANEQUAL, (),
          Node(MOANodeTypes.ARRAY, (), 'A'),
          Node(MOANodeTypes.PLUS, (),
               Node(MOANodeTypes.ARRAY, (), 'A'),
               Node(MOANodeTypes.ARRAY, (), 'B'))),
     '(A >= (A + B))'),
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (), (4,)),
      'B': SymbolNode(MOANodeTypes.ARRAY, (), (3,))},
     Node(MOANodeTypes.AND, (),
          Node(MOANodeTypes.LESSTHAN, (),
               Node(MOANodeTypes.ARRAY, (), 'A'),
               Node(MOANodeTypes.ARRAY, (), 'B')),
          Node(MOANodeTypes.EQUAL, (),
               Node(MOANodeTypes.ARRAY, (), 'B'),
               Node(MOANodeTypes.ARRAY, (), 'A'))),
     '((A < B) and (B == A))'),
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (), (4,)),
      'B': SymbolNode(MOANodeTypes.ARRAY, (), (3,))},
     Node(MOANodeTypes.OR, (),
          Node(MOANodeTypes.LESSTHAN, (),
               Node(MOANodeTypes.ARRAY, (), 'A'),
               Node(MOANodeTypes.ARRAY, (), 'B')),
          Node(MOANodeTypes.EQUAL, (),
               Node(MOANodeTypes.ARRAY, (), 'B'),
               Node(MOANodeTypes.ARRAY, (), 'A'))),
     '((A < B) or (B == A))'),
])
def test_python_backend_unit(symbol_table, tree, expected_source):
    assert expected_source == generate_python_source(symbol_table, tree)


def test_python_backend_materialize_scalar():
    symbol_table = {'A': SymbolNode(MOANodeTypes.ARRAY, (), (4,)),
                    'B': SymbolNode(MOANodeTypes.ARRAY, (), (3,))}

    tree = Node(MOANodeTypes.OR, (),
                Node(MOANodeTypes.LESSTHAN, (),
                     Node(MOANodeTypes.ARRAY, (), 'A'),
                     Node(MOANodeTypes.ARRAY, (), 'B')),
                Node(MOANodeTypes.EQUAL, (),
                     Node(MOANodeTypes.ARRAY, (), 'B'),
                     Node(MOANodeTypes.ARRAY, (), 'A')))

    assert generate_python_source(symbol_table, tree, materialize_scalars=True) == '((4 < 3) or (3 == 4))'


@pytest.mark.parametrize('symbol_table, tree, expected_source', [
    # Lenore Simple Example #1 06/01/2018
    ({'A': SymbolNode(MOANodeTypes.ARRAY, (3, 4), None),
      'B': SymbolNode(MOANodeTypes.ARRAY, (3, 4), None),
      '_i3': SymbolNode(MOANodeTypes.INDEX, (), (0, 3)),
      '_a4': SymbolNode(MOANodeTypes.ARRAY, shape=(2,), value=(Node(MOANodeTypes.ARRAY, (), '_i3'), 0))},
     Node(MOANodeTypes.PLUS, (3,),
          Node(MOANodeTypes.PSI, (3,),
               Node(MOANodeTypes.ARRAY, (2,), '_a4'),
               Node(MOANodeTypes.ARRAY, (3, 4), 'A')),
          Node(MOANodeTypes.PSI, (3,),
               Node(MOANodeTypes.ARRAY, (2,), '_a4'),
               Node(MOANodeTypes.ARRAY, (3, 4), 'B'))),
     "(A[(_i3, 0)] + B[(_i3, 0)])"
    )
])
def test_python_backend_integration(symbol_table, tree, expected_source):
    assert expected_source == generate_python_source(symbol_table, tree)
